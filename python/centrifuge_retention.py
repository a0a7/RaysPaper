...
with open('python/data/saline_absorbtion.csv','r') as data:
    ...
    for row in plots:
        ...
        if counter > 4:
            aval= float(row[1])
            a.append(aval)
            aError.append(float(row[2]))
        counter += 1
    data.close()
plt.title("")

plt.xlabel("Sample")
plt.ylabel(r'Centrifuge Retention $(g_{saline}/g_{dry}$ %)')
...
plt.bar(x, y, color='#35424a', label="Extruded Filaments", yerr=yError, ecolor='black', capsize=10, zorder=2, width=0.3, align='center', alpha=0.8)
plt.bar(b, a, color='#b30000', label="Hot-Pressed Sheets", yerr=aError, ecolor='black', capsize=10, zorder=2, width=0.3, align='edge', alpha=0.8)
